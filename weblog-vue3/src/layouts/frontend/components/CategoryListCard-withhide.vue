<template>
    ...
    <div
        ...
        <div
            ...
            <a @click="goCategoryArticleListPage(category.id,category.name)" v-for="(category, index) in visibleCategories" :key="index"
            ...
            <a @click="goCategoryArticleListPage(category2.id,category2.name)"  v-if="!moreCategoriesVisible"  v-for="(category2, index2) in showMoreCategories" :key="index2" 
                class="flex items-center w-full px-4 py-2 rounded-lg border-gray-200 cursor-pointer hover:bg-gray-100 hover:text-blue-700 dark:border-gray-600 dark:hover:bg-gray-600 dark:hover:text-white dark:focus:ring-gray-500 dark:focus:text-white">
                <svg class="w-3.5 h-3.5 mr-1.5 dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg"
                    fill="none" viewBox="0 0 21 18">
                    <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
...
//跳转到分类文章列表页
const goCategoryArticleListPage = (categoryId,name) =>{
    console.log("分页跳转---------》"+categoryId)
    router.push({path: '/category/article/list',query:{categoryId,name}})
}
//所有分类
